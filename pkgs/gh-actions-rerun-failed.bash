failed=0
repos_output=$(gh repo list josh --json 'nameWithOwner' --no-archived --limit 1000 --jq '.[] | .nameWithOwner')
repos=()
...
for repo in "${repos[@]}"; do
  ...
  for workflow in "${workflows[@]}"; do
    runs_output=$(gh run list --repo "$repo" --workflow "$workflow" --branch main --limit 1 --json 'databaseId,conclusion' --jq 'map(select(.conclusion == "failure"))[] | .databaseId')
    runs=()
