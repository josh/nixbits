...
stdenvNoCC.mkDerivation {
  ...
  buildCommand = ''
    ...
    makeWrapper $binPath $out/share/nix/hooks/pre-install.d/scutil --add-flags "--dry-run" --add-flags "$(printf '%q' "$computerName")"
    makeWrapper $binPath $out/share/nix/hooks/post-install.d/scutil --add-flags "$(printf '%q' "$computerName")"
  '';
