...
script.overrideAttrs (
  finalAttrs: _previousAttrs: {
    passthru.tests = {
      install-dry-run =
        runCommand "test-install-dry-run" { nativeBuildInputs = [ finalAttrs.finalPackage ]; }
          ''
      ...
      install-drv = runCommand "test-install-drv" { nativeBuildInputs = [ finalAttrs.finalPackage ]; } ''
        mkdir -p "$TMPDIR/Applications"
      ...
      install-app = runCommand "test-install-app" { nativeBuildInputs = [ finalAttrs.finalPackage ]; } ''
        mkdir -p "$TMPDIR/Applications"
      ...
      install-logging =
        runCommand "test-install-logging" { nativeBuildInputs = [ finalAttrs.finalPackage ]; }
          ''
            mkdir -p "$TMPDIR/Applications"
